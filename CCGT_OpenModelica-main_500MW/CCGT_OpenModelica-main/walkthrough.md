# Walkthrough — `OpenLoopCombineCycle_M701F_500MW`

## What Was Done

A new model `OpenLoopCombineCycle_M701F_500MW` was added to the `Simulators` package, based on **Mitsubishi M701F** gas turbine data and targeting a **2:1 GT:ST power split** (~333 MW GT / ~167 MW ST).

[CombineCycle.mo](file:///c:/Users/user/OneDrive/Documents/CCGT_OpenModelica-main%20-%20Copy/CCGT_OpenModelica-main%20-%20Copy/ThermoPower/ThermoPower/CombineCycle.mo)

```diff:CombineCycle.mo
within ThermoPower;
...
// ==== ST Connections ====
      ...
      annotation(
        experiment(StopTime = 1000, Tolerance = 1e-06),
        Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-520, 180}, {320, -240}}, initialScale = 0.1)),
        Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, initialScale = 0.1), graphics = {Rectangle(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 255}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid), Text(extent = {{-88, 84}, {100, -96}}, lineColor = {0, 0, 255}, textString = "500MW")}),
        Documentation(info = "<html><p>500 MW combined cycle power plant — open-loop simulation with Malaysian equatorial ambient conditions (32 degC, 1.01325 bar). GT ~330 MW, ST ~170 MW. Single-pressure HRSG at 80 bar.</p></html>"));
    end OpenLoopCombineCycle_500MW;

    model OpenLoopCombineCycle_M701F_500MW "500 MW CCGT based on M701F — 2:1 GT:ST split, Malaysian ambient (32 degC)"
      extends Modelica.Icons.Example;
      import ThermoPower;

      // ============================================================
      //  Brayton Cycle (GT) — based on Mitsubishi M701F
      //    ISO: 385 MW, PR 21:1, exhaust 748 kg/s @ 630 degC
      //    Malaysian ambient 32 degC derates to ~333 MW
      // ============================================================

      // Efficiency maps — unchanged shape, valid across sizes
      parameter Real tableEtaC[6, 4] = [0, 95, 100, 105; 1, 82.5e-2, 81e-2, 80.5e-2; 2, 84e-2, 82.9e-2, 82e-2; 3, 83.2e-2, 82.2e-2, 81.5e-2; 4, 82.5e-2, 81.2e-2, 79e-2; 5, 79.5e-2, 78e-2, 76.5e-2];
      parameter Real tableEtaT[5, 4] = [1, 90, 100, 110; 2.36, 89e-2, 89.5e-2, 89.3e-2; 2.88, 90e-2, 90.6e-2, 90.5e-2; 3.56, 90.5e-2, 90.6e-2, 90.5e-2; 4.46, 90.2e-2, 90.3e-2, 90e-2];

      // Pressure-ratio map — scaled to PR ~21 at design point (from 27)
      parameter Real tablePR[6, 4] = [0, 95, 100, 105; 1, 17.6, 21.0, 24.9; 2, 17.1, 20.7, 24.0; 3, 16.2, 19.8, 22.6; 4, 14.8, 18.9, 21.1; 5, 13.2, 16.7, 18.8];

      // Flow-capacity maps — scaled x2.8 for M701F air flow (~700 kg/s at 32 degC)
      parameter Real tablePhicC[6, 4] = [0, 95, 100, 105; 1, 107.2e-3, 120.4e-3, 131.0e-3; 2, 110.0e-3, 122.6e-3, 134.1e-3; 3, 113.7e-3, 126.6e-3, 135.5e-3; 4, 116.5e-3, 129.1e-3, 136.9e-3; 5, 118.4e-3, 130.5e-3, 138.0e-3];
      parameter Real tablePhicT[5, 4] = [1, 90, 100, 110; 2.36, 13.10e-3, 13.10e-3, 13.10e-3; 2.88, 13.10e-3, 13.10e-3, 13.10e-3; 3.56, 13.10e-3, 13.10e-3, 13.10e-3; 4.46, 13.10e-3, 13.10e-3, 13.10e-3];

      replaceable package FlueGas = ThermoPower.Media.FlueGas constrainedby Modelica.Media.Interfaces.PartialMedium "Flue gas model";
      replaceable package Water = ThermoPower.Water.StandardWater constrainedby Modelica.Media.Interfaces.PartialPureSubstance "Fluid model";

      // ---- GT Components ----
      // Malaysian equatorial ambient: 32 degC (305.15 K), sea-level
      Gas.SourcePressure SourceP1(redeclare package Medium = Media.Air, p0 = 1.01325e5, T = 305.15) annotation(
        Placement(transformation(origin = {-298, 20}, extent = {{-188, -30}, {-168, -10}})));
      Gas.Compressor compressor(redeclare package Medium = Media.Air, tablePhic = tablePhicC, tableEta = tableEtaC, pstart_in = 1.01325e5, pstart_out = 2.13e6, Tstart_in = 305.15, tablePR = tablePR, Table = ThermoPower.Choices.TurboMachinery.TableTypes.matrix, Tstart_out = 680, explicitIsentropicEnthalpy = true, Tdes_in = 305.15, Ndesign = 157.08) annotation(
        Placement(transformation(origin = {-286, 10}, extent = {{-158, -90}, {-98, -30}})));
      Gas.Turbine turbine(redeclare package Medium = FlueGas, pstart_in = 2.05e6, pstart_out = 1.05e5, tablePhic = tablePhicT, tableEta = tableEtaT, Table = ThermoPower.Choices.TurboMachinery.TableTypes.matrix, Tstart_out = 903, Tdes_in = 1400, Tstart_in = 1370, Ndesign = 157.08) annotation(
        Placement(transformation(origin = {-326, 16}, extent = {{-6, -90}, {54, -30}})));
      Gas.CombustionChamber CombustionChamber1(gamma = 1, Cm = 1, pstart = 2.10e6, Tstart = 1370, V = 0.15, S = 0.15, initOpt = ThermoPower.Choices.Init.Options.steadyState, HH = 41.6e6) annotation(
        Placement(transformation(origin = {-306, 14}, extent = {{-72, 20}, {-32, 60}})));
      Gas.SourceMassFlow SourceW1(redeclare package Medium = Media.NaturalGas, w0 = 14.5, p0 = 2200000, T = 300, use_in_w0 = true) annotation(
        Placement(transformation(origin = {-276, 18}, extent = {{-100, 70}, {-80, 90}})));
      Gas.PressDrop PressDrop1(redeclare package Medium = FlueGas, FFtype = ThermoPower.Choices.PressDrop.FFtypes.OpPoint, wnom = 720, rhonom = 5, dpnom = 20000, pstart = 2080000, Tstart = 1370) annotation(
        Placement(transformation(origin = {-326, 18}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));
      Gas.PressDrop PressDrop2(pstart = 2.13e6, FFtype = ThermoPower.Choices.PressDrop.FFtypes.OpPoint, A = 1, redeclare package Medium = Media.Air, dpnom = 0.15e5, wnom = 700, rhonom = 12, Tstart = 680) annotation(
        Placement(transformation(origin = {-390, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
      Modelica.Mechanics.Rotational.Sensors.PowerSensor powerSensor_GT annotation(
        Placement(transformation(origin = {-322, 10}, extent = {{60, -70}, {80, -50}})));
      Modelica.Mechanics.Rotational.Sources.ConstantSpeed constantSpeed_GT(w_fixed = 157, phi(start = 0, fixed = true)) annotation(
      ...
      // ============================================================
      //  Steam Cycle (Rankine) — sized for ~167 MW, 80 bar live steam
      //  HRSG surfaces x2 (moderate sizing for 2:1 GT:ST split)
      // ============================================================
      Models.PrescribedPresureCondenser condenser(p = 5390, Vtot = 20, redeclare package Medium = Water, initOpt = ThermoPower.Choices.Init.Options.fixedState) annotation(
        Placement(transformation(origin = {-32, 0}, extent = {{100, -100}, {140, -60}})));
      Models.PrescribedSpeedPump prescribedSpeedPump(n0 = 1500, nominalMassFlowRate = 100, q_nom = {0, 0.10, 0.18}, redeclare package FluidMedium = Water, head_nom = {1200, 800, 0}, rho0 = 1000, nominalOutletPressure = 8000000, nominalInletPressure = 50000) annotation(
        Placement(transformation(extent = {{40, -180}, {0, -140}})));
      Modelica.Blocks.Continuous.FirstOrder nPumpActuator(k = 1, initType = Modelica.Blocks.Types.Init.SteadyState, T = 2, y_start = 1500) annotation(
        Placement(transformation(origin = {20, -60}, extent = {{-280, -110}, {-260, -90}})));
      Modelica.Blocks.Interfaces.RealOutput generatedPower_ST annotation(
      ...
      // Steam turbine — 100 kg/s, 80 bar inlet
      ThermoPower.Water.SteamTurbineStodola steamTurbine(wstart = 100, wnom = 100, Kt = 0.007, redeclare package Medium = Water, PRstart = 50, pnom = 8000000) annotation(
        Placement(transformation(origin = {-48, -4}, extent = {{50, 30}, {100, 80}})));
      Modelica.Mechanics.Rotational.Sensors.PowerSensor powerSensor_ST annotation(
        Placement(transformation(origin = {-34, -2}, extent = {{138, 68}, {166, 40}})));

      // Economizer — surfaces x2, 80 bar, 750 kg/s gas nominal
      Models.HE economizer(redeclare package FluidMedium = Water, redeclare package FlueGasMedium = FlueGas, N_F = 6, exchSurface_G = 80190, exchSurface_F = 6879, extSurfaceTub = 7777, gasVol = 20, fluidVol = 58, metalVol = 16.1, rhomcm = 7900*578.05, lambda = 20, gasNomFlowRate = 750, fluidNomFlowRate = 100, gamma_G = 30, gamma_F = 3000, rhonom_G = 1, Kfnom_F = 150, FFtype_G = ThermoPower.Choices.Flow1D.FFtypes.OpPoint, FFtype_F = ThermoPower.Choices.Flow1D.FFtypes.Kfnom, N_G = 6, gasNomPressure = 101325, fluidNomPressure = 8000000, Tstart_G = 493.15, Tstart_M = 443.15, dpnom_G = 1000, dpnom_F = 40000) annotation(
        Placement(transformation(extent = {{-120, -80}, {-80, -120}})));

      // Evaporator — surfaces x2, 80 bar (Tsat ~ 568 K)
      Models.Evaporator evaporator(redeclare package FluidMedium = Water, redeclare package FlueGasMedium = FlueGas, gasVol = 20, fluidVol = 24.8, metalVol = 9.6, gasNomFlowRate = 750, fluidNomFlowRate = 100, N = 4, rhom = 7900, cm = 578.05, gamma = 85, exchSurface = 48804, gasNomPressure = 101325, fluidNomPressure = 8000000, Tstart = 568.15, FFtype_G = ThermoPower.Choices.Flow1D.FFtypes.OpPoint, dpnom_G = 1000, rhonom_G = 1) annotation(
        Placement(transformation(extent = {{-120, 0}, {-80, -40}})));

      // Superheater — surfaces x2, 80 bar steam
      Models.HE superheater(redeclare package FluidMedium = Water, redeclare package FlueGasMedium = FlueGas, N_F = 7, exchSurface_G = 4630, exchSurface_F = 900, extSurfaceTub = 1010, gasVol = 20, fluidVol = 8.9, metalVol = 2.3, rhomcm = 7900*578.05, lambda = 20, gasNomFlowRate = 750, gamma_G = 90, gamma_F = 6000, fluidNomFlowRate = 100, rhonom_G = 1, Kfnom_F = 150, FluidPhaseStart = ThermoPower.Choices.FluidPhase.FluidPhases.Steam, FFtype_G = ThermoPower.Choices.Flow1D.FFtypes.OpPoint, FFtype_F = ThermoPower.Choices.Flow1D.FFtypes.Kfnom, N_G = 7, gasNomPressure = 101325, fluidNomPressure = 8000000, Tstart_G = 823.15, Tstart_M = 673.15, dpnom_G = 1000, dpnom_F = 40000) annotation(
        Placement(transformation(extent = {{-120, 80}, {-80, 40}})));
      ...
      // ---- System with Malaysian ambient conditions ----
      ...
      // Fuel step: offset 14.5 kg/s, +2.0 kg/s step at t=500 s
      Modelica.Blocks.Sources.Step fuelFlowRateStep(height = 2.0, startTime = 500, offset = 14.5) annotation(
        Placement(transformation(origin = {-72, 0}, extent = {{-440, 96}, {-420, 116}})));
      Modelica.Blocks.Sources.Step voidFractionSetPoint(offset = 0.2, height = 0, startTime = 0) annotation(
        Placement(transformation(origin = {-12, -46}, extent = {{-360, -120}, {-340, -100}})));
      Models.PID voidFractionController(PVmin = 0.1, PVmax = 0.9, CSmax = 2500, PVstart = 0.1, CSstart = 0.5, steadyStateInit = true, CSmin = 500, Kp = -2, Ti = 300) annotation(
    ...
    equation
      ...
      annotation(
        experiment(StopTime = 1000, Tolerance = 1e-06),
        Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-520, 180}, {320, -240}}, initialScale = 0.1)),
        Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}, initialScale = 0.1), graphics = {Rectangle(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 255}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid), Text(extent = {{-88, 84}, {100, -96}}, lineColor = {0, 0, 255}, textString = "M701F")}),
        Documentation(info = "<html><p>500 MW CCGT based on Mitsubishi M701F gas turbine. Malaysian equatorial ambient (32 degC dry bulb, 28 degC wet bulb, 1.01325 bar). Target: GT ~333 MW, ST ~167 MW (2:1 split). Single-pressure HRSG at 80 bar. PR ~21:1.</p></html>"));
    end OpenLoopCombineCycle_M701F_500MW;
...
## Root Cause of Previous 50/50 Split

The previous `OpenLoopCombineCycle_500MW` model had two problems:

1. **HRSG was oversized** — surfaces scaled ×3 captured too much GT exhaust heat → ST produced as much as GT
2. **PR too high** (27:1 from original maps) — high compressor work consumed GT shaft output, reducing net GT power
3. **System block ambient** left at defaults (T_amb=20°C, T_wb=15°C), not Malaysian conditions

## Fixes Applied in M701F Model

### 1. System Block — Malaysian Ambient ✅
```modelica
inner System system(
  allowFlowReversal = false,
  initOpt = ThermoPower.Choices.Init.Options.steadyState,
  T_amb = 305.15,   // 32°C — Malaysian dry bulb
  T_wb = 301.15     // 28°C — tropical wet bulb (~80% RH)
)
```

Parameters confirmed from [ThermoPower/package.mo](file:///c:/Users/user/OneDrive/Documents/CCGT_OpenModelica-main%20-%20Copy/CCGT_OpenModelica-main%20-%20Copy/ThermoPower/ThermoPower/package.mo#L19-L24):
- `T_amb` (line 21): default 293.15 K → now **305.15 K** (32°C)
- `T_wb` (line 23): default 288.15 K → now **301.15 K** (28°C)

### 2. GT — M701F Specifications

| Parameter | Prev 500MW | M701F Model | M701F Real |
|-----------|-----------|-------------|------------|
| PR (design) | 27:1 | **21:1** | 21:1 |
| Flow tables | ×3.0 | **×2.8** | ~748 kg/s exhaust |
| Fuel flow | 19.2 kg/s | **14.5 kg/s** | ~14.5 kg/s |
| Exhaust temp (Tstart_out) | 800 K | **903 K** (630°C) | 630°C |
| Compressor pstart_out | 2.45 MPa | **2.13 MPa** | ~21 bar |
| Turbine pstart_in | 2.38 MPa | **2.05 MPa** | — |
| CC pstart | 2.41 MPa | **2.10 MPa** | — |

> [!NOTE]
> Reducing PR from 27 to 21 cuts compressor work significantly → more net GT shaft power → GT produces larger share of total output.

### 3. HRSG — Reduced from ×3 to ×2

| HE | Surface scaling | gasNomFlowRate | fluidNomFlowRate |
|----|----------------|----------------|------------------|
| Economizer | ×2.0 | 750 | 100 |
| Evaporator | ×2.0 | 750 | 100 |
| Superheater | ×2.0 | 750 | 100 |

Superheater `Tstart_G` raised to **823 K** (from 773 K) to reflect hotter M701F exhaust.

### 4. Steam Cycle — Right-sized for 167 MW

| Parameter | Prev 500MW | M701F Model |
|-----------|-----------|-------------|
| Steam flow (wnom) | 165 kg/s | **100 kg/s** |
| Kt (Stodola) | 0.0117 | **0.007** |
| Pump q_nom | {0, 0.165, 0.3} | **{0, 0.10, 0.18}** |
| Pump nominalMassFlowRate | 165 | **100** |
| Condenser Vtot | 30 m³ | **20 m³** |

---

## Verification

Load `ThermoPower.CombineCycle.Simulators.OpenLoopCombineCycle_M701F_500MW` in OpenModelica:
1. Simulate for 1000 s
2. Check `generatedPower_GT` ≈ 333 MW (2/3 of 500)
3. Check `generatedPower_ST` ≈ 167 MW (1/3 of 500)
4. Verify fuel step at t=500 s produces a proportional GT response
5. Verify System block shows T_amb=305.15 K, T_wb=301.15 K in parameters dialog
